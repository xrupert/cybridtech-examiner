...
function testMultipleMortgagesRequireTargetSelectionWhenSummaryIncomplete() {
  ...
  const { record, review } = build(raw); assert.equal(record.mortgages.length, 2); assert.equal(record.targetLien.selectionRequired, true); assert.equal(record.targetLien.position.value, "Needs review"); const warnings = validateExportProfile(MCCALLA_EXPORT_PROFILE, [{ record: review.record, qc: review.qc }]); assert.ok(warnings.some((warning) => /Lien Amount|Lien Position/.test(warning)));
}
...
function testFirstInTimeAcrossOpenLienStack() {
  ...
  const { record } = build(raw); assert.equal(record.targetLien.position.value, "2nd Lien"); assert.equal(record.foreclosureAnalysis.seniorLienIds.length, 1); assert.ok(record.foreclosureAnalysis.requirements.some((item) => item.type === "PAYOFF_REVIEW"));
}
...
function testStatutoryPriorityExceptionIsFlagged() {
  ...
  const { record } = build(raw); assert.equal(record.targetLien.position.value, "2nd Lien"); assert.ok(record.foreclosureAnalysis.requirements.some((item) => item.code.startsWith("STACK_EXCEPTION_") && item.type === "PRIORITY_REVIEW"));
}
...
function testCurrentOwnerKeepsVera20ButExcludesForeclosureOnlyChecks() {
  ...
  assert.equal(qc.checks.filter((check) => check.legacyQuestionNumber).length, 20); assert.equal(record.targetLien.amount.value, "$80,000"); assert.equal(record.targetLien.position.value, "1st Lien");
}
...
function waRecord(withJunior = false, withFederal = false) {
  ...
  const { record } = build(raw); const analysis = jurisdictionAnalysisForRecord(record); record.foreclosureAnalysis = mergeJurisdictionRequirements(record, analysis); return record;
}
