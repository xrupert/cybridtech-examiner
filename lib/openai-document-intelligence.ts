...
import { emptyVera, type AuditFinding, type EvidenceRef, type EvidenceSource, type FieldEvidence, type FindingStatus, type PacketDocument, type PageEvidence, type VeraExam } from "./vera";
...
async function responseAudit(args: { fileId?: string; text?: string; pageMarkedText?: boolean; context: AuditContext; model: string }): Promise<{ raw: RawAudit; modelMs: number }> {
  const started = Date.now();
  const promptText = args.fileId
    ? "Review the attached title packet now and return the final VERA audit."
    : args.pageMarkedText
      ? `Review this complete extracted title packet. Preserve the physical page numbers shown in the === PDF PAGE N === markers.\n\n${args.text || ""}`
      : `Review this title-report text as Page 1:\n\n${args.text || ""}`;
  ...
  console.info("CYBRID_TITLE_MODEL_PASS", JSON.stringify({
    ...
    model: args.model,
    inputMode: args.fileId ? "pdf-file" : args.pageMarkedText ? "page-addressable-text" : "text",
    effort: "low",
    ms: modelMs,
  ...
  try {
    return { raw: JSON.parse(text) as RawAudit, modelMs };
  } catch {
...
function uniqueEvidence(items: RawEvidence[], sourceFile: string, source: EvidenceSource): EvidenceRef[] {
...
function mapFindings(raw: RawFinding[], sourceFile: string, source: EvidenceSource): AuditFinding[] {
  ...
  return REQUIRED_QUESTIONS.map((question, index) => {
    ...
    if (!item) return { number, question, critical: CRITICAL_QUESTION_NUMBERS.has(number), response: "Cannot Confirm", status: "CANNOT_CONFIRM", evidence: [], proofReason: "The model did not return this required VERA finding.", commentary: "Server structural gate supplied the missing question." };
    return { number, question, critical: CRITICAL_QUESTION_NUMBERS.has(number), response: item.response || "Not Provided", status: item.status, evidence: uniqueEvidence(item.evidence || [], sourceFile, source), proofReason: item.proofReason || "Not Stated", commentary: item.commentary || "" };
  });
}

function mapSummaryEvidence(raw: RawFieldEvidence[], sourceFile: string, source: EvidenceSource): FieldEvidence[] {
  return (raw || []).map((item) => ({ field: item.field, value: item.value, status: item.status, evidence: uniqueEvidence(item.evidence || [], sourceFile, source), proofReason: item.proofReason }));
}

function mapPages(raw: RawAudit["pages"], source: EvidenceSource): PageEvidence[] {
  const map = new Map<number, PageEvidence>();
  for (const page of raw || []) {
    if (!page.page || page.page < 1) continue;
    if (!map.has(page.page)) map.set(page.page, { page: page.page, text: page.excerpt || "", source, documentType: page.documentType || "Unclassified" });
  }
  return [...map.values()].sort((a, b) => a.page - b.page);
}

function mapDocuments(raw: RawAudit["documents"]): PacketDocument[] {
  return (raw || []).map((doc) => ({ documentType: doc.documentType || "Unclassified", pageStart: Math.max(1, doc.pageStart || 1), pageEnd: Math.max(doc.pageStart || 1, doc.pageEnd || doc.pageStart || 1), instrumentNumber: doc.instrumentNumber && doc.instrumentNumber !== "Not Stated" ? doc.instrumentNumber : undefined, recordingDate: doc.recordingDate && doc.recordingDate !== "Not Stated" ? doc.recordingDate : undefined, excerpt: doc.excerpt || "" })).sort((a, b) => a.pageStart - b.pageStart);
}
...
function normalizeLoanStatus(value: string): VeraExam["loanStatus"] { const lower = value.toLowerCase(); if (lower.includes("default") || lower.includes("foreclos")) return "Default"; if (lower.includes("satisfied") || lower.includes("released") || lower.includes("paid")) return "Satisfied"; if (lower.includes("active") || lower.includes("open") || lower.includes("current")) return "Active"; return "Not Provided"; }

function finalize(raw: RawAudit, context: AuditContext, model: string, source: EvidenceSource, engine: Partial<VeraExam["documentEngine"]> = {}): VeraExam {
  const findings = mapFindings(raw.findings, context.sourceFile, source);
  const pages = mapPages(raw.pages, source);
  const documents = mapDocuments(raw.documents);
  return emptyVera({
    state: context.state || "TX", county: raw.header.county || "Not Stated", searchType: context.searchType || "Foreclosure", clientOrder: raw.header.clientOrder || "Not Provided", propertyAddress: raw.header.propertyAddress || "Not Provided", searchEffectiveDate: raw.header.searchEffectiveDate || "Not Provided", minNumber: raw.header.minNumber || "Not Provided", parcelId: raw.header.parcelId || "Not Provided", landValue: raw.header.landValue || "Not Provided", improvements: raw.header.improvements || "Not Provided", taxStatus: raw.header.taxStatus || "Not Provided", fiscalYear: raw.header.fiscalYear || "Not Provided", mobileHome: raw.header.mobileHome, condoHoa: raw.header.condoHoa,
    hoaPresent: q(findings, 1), ccrs: q(findings, 2), hoaNameAmounts: q(findings, 3), deedMortgageAccurate: q(findings, 4), deed: raw.deed, mortgages: raw.mortgages, recordingsAvailable: q(findings, 5), recordingsChronological: q(findings, 6), assignmentVesting: q(findings, 7), legalDescriptionConfirmed: q(findings, 8), legalDescription: raw.header.legalDescription || "Not Provided", originalBeneficiaryMers: q(findings, 9), federalTaxLien: q(findings, 10), documentReleases: q(findings, 11), propertySecuredAddressMatch: q(findings, 12), loanDocumentType: normalizeLoanDocumentType(q(findings, 13)), recordingDate: q(findings, 14), loanStatus: normalizeLoanStatus(q(findings, 15)), recourse: q(findings, 16), typosOrErrors: q(findings, 17), platMapLabeled: q(findings, 18), minInRunSheet: q(findings, 19), runSheetAccurate: q(findings, 20),
    audit: raw.audit, summaryEvidence: mapSummaryEvidence(raw.summaryEvidence, context.sourceFile, source), findings, pages, documents, manualReviewRequired: raw.manualReviewRequired,
    extractionSummary: `${model} completed one full-packet forensic audit from ${source === "native" ? "a page-addressable native extraction ledger" : source === "openai-file" ? "the source PDF/vision path" : "provided text"}. Cybrid Title then applies its deterministic evidence/structure critic before final PASS/FAIL.`,
    rulePackStatus: "VERA v3 + RCS Foreclosure/2nd Lien/Current Owner rules + Quick Reference Checklist + Legal Description Compliance Protocol loaded.", notes: raw.notes || raw.extractionSummary || "", sourceFile: context.sourceFile, extractedAt: new Date().toISOString(), rawExcerpt: pages.map((page) => `P${page.page}: ${page.text}`).join("\n").slice(0, 1800),
    packetHash: engine.packetHash || "", packetPageCount: engine.pageCount || 0,
    documentEngine: {
      packetHash: engine.packetHash || "", pageCount: engine.pageCount || 0, extractionMode: engine.extractionMode || (source === "native" ? "native-text" : source === "openai-file" ? "openai-pdf-fallback" : "pasted-text"), extractionCacheHit: engine.extractionCacheHit || false, textCoverage: engine.textCoverage || 0, lowTextPages: engine.lowTextPages || [], extractionMs: engine.extractionMs || 0, modelMs: engine.modelMs || 0,
    },
  });
}
...
export async function analyzePdfWithOpenAI(buffer: ArrayBuffer, context: AuditContext): Promise<VeraExam> {
  ...
  const prepared = await preparePdfPacket(buffer, context.sourceFile);
  const model = openAIDocumentModel();

  if (prepared.extractionMode === "native-text" && prepared.pageDelimitedText) {
    const result = await responseAudit({ text: prepared.pageDelimitedText, pageMarkedText: true, context, model });
    const exam = finalize(result.raw, context, model, "native", {
    ...
    console.info("CYBRID_TITLE_FORENSIC_COMPLETE", JSON.stringify({ sourceFile: context.sourceFile, packetHash: prepared.packetHash, model, extractionMode: "native-text", cacheHit: prepared.cacheHit, pageCount: prepared.ledger.pageCount, ms: Date.now() - overallStarted }));
    return exam;
  }

  const uploadStarted = Date.now();
  const fileId = await uploadPdf(buffer, context.sourceFile);
  console.info("CYBRID_TITLE_FILE_UPLOAD", JSON.stringify({ sourceFile: context.sourceFile, packetHash: prepared.packetHash, ms: Date.now() - uploadStarted }));
  try {
    const result = await responseAudit({ fileId, context, model });
    const exam = finalize(result.raw, context, model, "openai-file", {
      packetHash: prepared.packetHash,
      pageCount: prepared.ledger.pageCount,
    ...
    console.info("CYBRID_TITLE_FORENSIC_COMPLETE", JSON.stringify({ sourceFile: context.sourceFile, packetHash: prepared.packetHash, model, extractionMode: "openai-pdf-fallback", cacheHit: prepared.cacheHit, pageCount: prepared.ledger.pageCount, ms: Date.now() - overallStarted }));
    return exam;
  } finally {
...
export async function analyzeTextWithOpenAI(text: string, context: AuditContext): Promise<VeraExam> {
  const started = Date.now();
  const model = openAIDocumentModel();
  const result = await responseAudit({ text, context, model });
  const exam = finalize(result.raw, context, model, "pasted", { extractionMode: "pasted-text", modelMs: result.modelMs });
  console.info("CYBRID_TITLE_FORENSIC_COMPLETE", JSON.stringify({ sourceFile: context.sourceFile, model, extractionMode: "pasted-text", ms: Date.now() - started }));
  return exam;
}
