...
function apiKey(): string { const key = process.env.OPENAI_API_KEY; if (!key) throw new Error("OpenAI is not configured. Add OPENAI_API_KEY to the Vercel project."); return key; }
...
async function deleteFile(fileId: string): Promise<void> {
  try { await fetch(`${OPENAI_API}/files/${fileId}`, { method: "DELETE", headers: { Authorization: `Bearer ${apiKey()}` } }); } catch { /* cleanup failure must not erase a completed audit */ }
}
...
async function responseAudit(args: { fileId?: string; text?: string; context: AuditContext; pass: "primary" | "verification"; model: string }): Promise<RawAudit> {
  ...
  try { return JSON.parse(text) as RawAudit; } catch { throw new Error("OpenAI returned structured output that could not be parsed as the forensic audit schema."); }
}
...
function uniqueEvidence(items: RawEvidence[]): EvidenceRef[] {
  ...
  for (const item of items || []) { const key = `${item.page}|${normalizeText(item.documentType)}|${normalizeText(item.quote)}`; if (seen.has(key)) continue; seen.add(key); out.push({ quote: item.quote || "Not Stated", page: Math.max(1, item.page || 1), documentType: item.documentType || "Unclassified", source: "openai-file", instrumentNumber: item.instrumentNumber && item.instrumentNumber !== "Not Stated" ? item.instrumentNumber : undefined }); }
  return out;
}
...
function reconcileFieldEvidence(primary: RawFieldEvidence[], verification: RawFieldEvidence[], disagreements: string[]): FieldEvidence[] {
  ...
  return (primary || []).map((item) => {
    const other = byField.get(normalizeText(item.field)); if (!other) return { ...item, evidence: uniqueEvidence(item.evidence || []) };
    const valueDiffers = normalizeText(item.value) !== normalizeText(other.value) && normalizeText(item.value) && normalizeText(other.value);
...
function reconcileFindings(primary: RawFinding[], verification: RawFinding[], disagreements: string[]): AuditFinding[] {
  const p = new Map((primary || []).map((item) => [item.number, item])); const v = new Map((verification || []).map((item) => [item.number, item]));
  return REQUIRED_QUESTIONS.map((question, index) => {
    const number = index + 1; const a = p.get(number); const b = v.get(number);
    if (!a || !b) { disagreements.push(`Q${number} missing from one model pass.`); return { number, question, critical: CRITICAL_QUESTION_NUMBERS.has(number), response: "Cannot Confirm", status: "CANNOT_CONFIRM", evidence: uniqueEvidence([...(a?.evidence || []), ...(b?.evidence || [])]), proofReason: "Independent audit passes did not both return this required question.", commentary: "" }; }
    if (a.status !== b.status) { disagreements.push(`Q${number} status disagreement: ${a.status} vs ${b.status}.`); return { number, question, critical: CRITICAL_QUESTION_NUMBERS.has(number), response: `${a.response} / Verification: ${b.response}`, status: "CANNOT_CONFIRM", evidence: uniqueEvidence([...(a.evidence || []), ...(b.evidence || [])]), proofReason: `Independent OpenAI audit passes disagreed. Primary: ${a.proofReason} Verification: ${b.proofReason}`, commentary: [a.commentary, b.commentary].filter(Boolean).join(" | ") }; }
...
function mergePages(primary: RawAudit["pages"], verification: RawAudit["pages"]): PageEvidence[] {
  ...
  for (const page of [...(primary || []), ...(verification || [])]) { const existing = map.get(page.page); const documentType = existing && existing.documentType !== "Unclassified" ? existing.documentType : page.documentType || "Unclassified"; const text = existing?.text || page.excerpt || ""; map.set(page.page, { page: page.page, text, source: "openai-file", documentType }); }
