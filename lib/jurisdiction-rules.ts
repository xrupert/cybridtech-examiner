...
function washington(record: CanonicalTitleRecord): JurisdictionAnalysis {
  ...
  if (record.foreclosureAnalysis.juniorLienIds.length) {
    requirements.push(requirement({
      record,
      lienIds: record.foreclosureAnalysis.juniorLienIds,
      ...
      authorityUrl: RCW_040,
      ruleVersion: WA_RULE_VERSION,
      title: `${record.foreclosureAnalysis.juniorLienIds.length} junior interest${record.foreclosureAnalysis.juniorLienIds.length === 1 ? "" : "s"} require Washington foreclosure-notice review.`,
      action: "Use the developed junior-lien stack to identify holders entitled to Notice of Trustee's Sale under Washington law. Confirm current record-holder identity/address and required mailing/service treatment before sale processing is finalized.",
    }));
  }
