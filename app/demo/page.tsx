...
function statusClass(value: string) { if (value === "CLEAR" || value === "PASS" || value === "NOT_APPLICABLE") return `${styles.status} ${styles.clear}`; if (value === "CURATIVE_REQUIRED" || value === "FAIL") return `${styles.status} ${styles.curative}`; if (value === "QC_DEFICIENCY") return `${styles.status} ${styles.qc}`; return `${styles.status} ${styles.review}`; }
...
function isForeclosureReview(review?: TitleReviewResult) { return Boolean(review && /^foreclosure$/i.test(review.record.orderType.value)); }
...
export default function DemoPage() {
  ...
  async function createBatch(): Promise<BatchManifest> { const response = await fetch("/api/batches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientName, sourceFiles: files.map((file) => file.name), exportProfileId: "ncala-demo-v1" }) }); return parseResponse(response) as Promise<BatchManifest>; }
  ...
  function selectTargetLien(item: BatchItem, instrumentId: string) {
    if (!item.review || !isForeclosureReview(item.review)) return;
    patchReview(item.id, (review) => {
      ...
      const beneficiary = mortgage.parties.find((party) => /holder|beneficiary|mortgagee|lender/i.test(party.role))?.name || "Needs review"; const confirmed = mortgage.evidence.length ? "CONFIRMED" as const : "UNCONFIRMED" as const;
      const record = { ...review.record, targetLien: { ...review.record.targetLien, instrumentId: mortgage.id, instrumentNumber: { value: mortgage.instrumentNumber, state: confirmed, evidence: mortgage.evidence, evidenceIds: mortgage.evidenceIds, basis: "Examiner selected foreclosure target lien" }, amount: { value: mortgage.amount, state: confirmed, evidence: mortgage.evidence, evidenceIds: mortgage.evidenceIds, basis: "Amount from examiner-selected foreclosure target lien" }, beneficiary: { value: beneficiary, state: confirmed, evidence: mortgage.evidence, evidenceIds: mortgage.evidenceIds, basis: "Beneficiary/holder on examiner-selected foreclosure target lien" }, selectionRequired: false } };
      const checks = review.qc.checks.map((check) => check.id === "TARGET_LIEN_FOUND" ? { ...check, status: "PASS" as const, summary: `Target lien selected by examiner: ${mortgage.instrumentNumber}.`, evidence: mortgage.evidence, evidenceIds: mortgage.evidenceIds } : check);
  ...
  function setLienPosition(item: BatchItem, value: string) {
    ...
    patchReview(item.id, (review) => {
      ...
      const checks = review.qc.checks.map((check) => check.id === "TARGET_LIEN_POSITION_ESTABLISHED" ? { ...check, status: normalized === "Needs review" ? "CANNOT_CONFIRM" as const : "PASS" as const, summary: normalized === "Needs review" ? "Lien position remains unresolved." : `Target lien position confirmed as ${normalized}.` } : check);
      return { ...review, record, qc: reduceQcChecks(review.qc, checks) };
    });
  }
  ...
  const exportWarnings = useMemo(() => { if (!rows.length) return []; const fieldWarnings = validateExportProfile(exportProfile("csv"), rows); const decisionWarnings = completeItems.flatMap((item) => { const count = unresolvedDecisions(item); return count ? [`${item.fileName}: ${count} exception${count === 1 ? "" : "s"} still require examiner disposition.`] : []; }); return [...fieldWarnings, ...decisionWarnings]; /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [rows, selectedColumns, completeItems, decisions, clientName, availableColumns]);
  ...
  return <div className={styles.shell}>
    <header className={styles.nav}><div className={styles.brand}><Logo height={32} /><span className={styles.brandName}>CYBRID TITLE</span></div><span className={styles.navTag}>Title QC · Curative · Client Data</span></header>
    <main className={styles.main}>
      <section className={styles.hero}><div><p className={styles.eyebrow}>Canonical title intelligence workbench</p><h1>Upload the title reports. Find what is wrong. See what needs action. Export the data.</h1></div><p>Every packet is isolated and follows the same source-first path: extract → classify → normalize → check → ground → human exceptions → client export.</p></section>
      <section className={`${styles.panel} ${styles.setupPanel}`}><div className={styles.setup}>
        ...
        <div className={styles.profileNote}>State is detected from each packet. Mixed-state and mixed-order batches are allowed.</div><div className={styles.profileNote}>{modelStatus}<br />{readiness?.engine || "canonical engine"}</div>
      </div></section>
      {error ? <div className={styles.errorBox}>{error}</div> : null}{notice ? <div className={styles.notice}>{notice}</div> : null}
      <section className={`${styles.panel} ${styles.uploadPanel}`}><div className={styles.drop}><div><h2>Upload title-report packets</h2><p>One PDF packet is one isolated QC job. Upload one or a batch.</p></div><div className={styles.actions}><label className={styles.secondary}>Choose PDFs<input hidden type="file" multiple accept="application/pdf,.pdf" onChange={(event) => chooseFiles(event.target.files)} /></label><button className={styles.primary} disabled={!files.length || busy || readiness?.openAIConfigured === false} onClick={runBatch}>{busy ? "Running QC…" : "Run Title QC"}</button></div></div>
      ...
      {items.length ? <><section className={styles.metrics}><div className={styles.metric}><span>Batch</span><strong>{metrics.total}</strong></div><div className={`${styles.metric} ${styles.metricClear}`}><span>Clear</span><strong>{metrics.clear}</strong></div><div className={`${styles.metric} ${styles.metricCurative}`}><span>Curative</span><strong>{metrics.curative}</strong></div><div className={`${styles.metric} ${styles.metricReview}`}><span>Cannot confirm</span><strong>{metrics.review}</strong></div><div className={styles.metric}><span>QC deficiency</span><strong>{metrics.qc}</strong></div></section>
        <section className={styles.panel}><div className={styles.sectionTitle}><div><h2>Batch results</h2><p>Open a row to inspect grounded checks and resolve actual exceptions.</p></div></div><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>TS / Order #</th><th>Order Profile</th><th>Borrower</th><th>Property</th><th>Target Lien</th><th>Lien Position</th><th>QC</th><th>Review Readiness</th><th>Curative / QC Issues</th><th>Source</th></tr></thead><tbody>
          {items.map((item) => item.review ? <tr key={item.id}><td><button className={styles.rowButton} onClick={() => setSelectedId(item.id)}>{item.review.record.tsNumber.value}</button></td><td>{item.review.qc.profileName}</td><td>{item.review.record.borrower.value}</td><td>{item.review.record.propertyAddress.value}</td>
            <td>{isForeclosureReview(item.review) ? (item.review.record.targetLien.selectionRequired ? <select className={styles.editInput} value="" onChange={(event) => selectTargetLien(item, event.target.value)}><option value="">Select target lien</option>{item.review.record.mortgages.map((mortgage) => <option value={mortgage.id} key={mortgage.id}>{mortgage.instrumentNumber} · {mortgage.amount}</option>)}</select> : item.review.record.targetLien.instrumentNumber.value) : "N/A for this order"}</td>
            <td>{isForeclosureReview(item.review) ? <input className={styles.editInput} value={item.review.record.targetLien.position.value === "Needs review" ? "" : item.review.record.targetLien.position.value} placeholder="Needs review" onChange={(event) => setLienPosition(item, event.target.value)} /> : "N/A for this order"}</td>
            <td><span className={statusClass(item.review.qc.qcStatus)}>{item.review.qc.qcStatus}</span></td><td><span className={statusClass(item.review.qc.foreclosureReadiness)}>{item.review.qc.foreclosureReadiness.replaceAll("_", " ")}</span></td><td><div className={styles.curativeList}>{item.review.qc.curativeIssues.slice(0, 3).map((issue) => <span className={styles.curativeItem} key={`${item.id}-${issue.code}`}>{issue.code}</span>)}{unresolvedDecisions(item) ? <b>{unresolvedDecisions(item)} decision(s) needed</b> : null}</div></td><td>{item.fileName}</td></tr> : <tr key={item.id}><td colSpan={10}>{item.fileName} — <span className={item.status === "error" ? `${styles.status} ${styles.error}` : `${styles.status} ${styles.review}`}>{item.status.toUpperCase()}</span>{item.error ? ` · ${item.error}` : ""}</td></tr>)}
        </tbody></table></div></section></> : null}

      {selected?.review ? <section className={styles.panel}>
        <div className={styles.sectionTitle}><div><h2>{selected.review.record.tsNumber.value} · Review & curative</h2><p>{selected.review.record.runSheet.detected ? `Separate Run Sheet / Abstractor Sheet detected · pages ${selected.review.record.runSheet.pageStart ?? "?"}-${selected.review.record.runSheet.pageEnd ?? "?"}` : "No separate Run Sheet / Abstractor Sheet supplied · Run Sheet check is N/A. The title report is reviewed separately against its source documents."}</p></div></div>
        <div className={styles.detailGrid}><div className={styles.summaryCard}><dl>
          ...
          {isForeclosureReview(selected.review) ? <><dt>Target lien</dt><dd>{selected.review.record.targetLien.instrumentNumber.value}</dd><dt>Lien position</dt><dd>{selected.review.record.targetLien.position.value}</dd></> : null}
