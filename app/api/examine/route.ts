...
import { detectSearchTypeFromText, preparePdfPacket, type PreparedPacket } from "@/lib/document-engine";
import { verifyExamGrounding, type GroundingSummary } from "@/lib/evidence-grounding";
...
import { buildTitleReviewResult, TITLE_REVIEW_ENGINE_VERSION } from "@/lib/title-review";
...
type ReviewContext = { searchType: string; state: string };
type ExaminedPacket = { exam: VeraExam; grounding?: GroundingSummary };
...
function auditContext(state: string, searchType: string, sourceFile: string) {
  ...
  if (!isSupportedSearchType(normalizedSearchType)) {
    throw new Error(`Unsupported MVP search type: ${normalizedSearchType}. Use ${SEARCH_TYPES.join(", ")}.`);
  }
  return { state: state.trim().toUpperCase() || "NOT STATED", searchType: normalizedSearchType, sourceFile };
}

function openingText(prepared: PreparedPacket): string {
  return prepared.ledger.pages
...
function detectStateFromText(text: string): { state: string | null; evidence: string } {
  ...
  const zipMatches = [...compact.toUpperCase().matchAll(/\b([A-Z]{2})\s+\d{5}(?:-\d{4})?\b/g)];
...
function resolvePreparedContext(prepared: PreparedPacket, sourceFile: string, requestedSearchType: string, requestedState: string): ReviewContext {
  const opening = openingText(prepared);
...
function resolveTextContext(text: string, sourceFile: string, requestedSearchType: string, requestedState: string): ReviewContext {
  let searchType = requestedSearchType;
  if (requestedSearchType === AUTO_DETECT_SEARCH_TYPE) {
...
async function examineFile(file: File, state: string, searchType: string): Promise<ExaminedPacket> {
  const name = file.name || "upload";
  if (name.toLowerCase().endsWith(".pdf")) {
    const buffer = await file.arrayBuffer();
    const prepared = await preparePdfPacket(buffer.slice(0), name);
    const resolved = resolvePreparedContext(prepared, name, searchType, state);
    const candidate = critique(await analyzePdfWithOpenAI(buffer, auditContext(resolved.state, resolved.searchType, name)));
    const grounded = verifyExamGrounding(candidate, prepared.ledger);
    return { exam: critique(grounded.exam), grounding: grounded.summary };
  }

  const text = await file.text();
  if (!text.trim()) throw new Error(`Could not read text from ${name}`);
  const resolved = resolveTextContext(text, name, searchType, state);
  return { exam: critique(await analyzeTextWithOpenAI(text, auditContext(resolved.state, resolved.searchType, name))) };
}
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    engine: TITLE_REVIEW_ENGINE_VERSION,
    ...
    documentEngine: {
      ...
      scannedPdfFallback: "openai-pdf-vision-current; page-selective visual ledger remains hardening work",
    ...
    reviewPolicy: {
      ...
      deterministicNativeGrounding: true,
      unknownsFailClosed: true,
      targetLienNeverInferredFromArrayOrder: true,
    },
  });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (ctype.includes("multipart/form-data")) {
      ...
      if (files.length > 1) return NextResponse.json({ code: "TOO_MANY_FILES", error: "Each review job accepts one complete title-report packet. Batch QC submits multiple independent jobs." }, { status: 400 });
      examined = await examineFile(files[0], state, searchType);
    } else if (ctype.includes("application/json")) {
      const body = (await req.json()) as { fixtureId?: string; text?: string; sourceFile?: string; state?: string; searchType?: string; blobPathnames?: string[]; clientName?: string };
      ...
      if (body.blobPathnames?.length) {
        if (body.blobPathnames.length !== 1) return NextResponse.json({ code: "TOO_MANY_FILES", error: "Each review job accepts one title packet. Batch QC submits one job per packet." }, { status: 400 });
        ...
        const resolved = resolveTextContext(fixture.text, fixture.name, searchType, state);
        examined = { exam: critique(await analyzeTextWithOpenAI(fixture.text, auditContext(resolved.state, resolved.searchType, fixture.name))) };
      } else if (body.text?.trim()) {
        const sourceFile = body.sourceFile || "pasted-text";
        const resolved = resolveTextContext(body.text, sourceFile, searchType, state);
        examined = { exam: critique(await analyzeTextWithOpenAI(body.text, auditContext(resolved.state, resolved.searchType, sourceFile))) };
      } else {
    ...
    const exam = await recordCompletedReview(examined.exam, openAIDocumentModel());
    const review = buildTitleReviewResult(exam, clientName);
    const usage = {
      reviews: 1,
      ...
      grounding: examined.grounding || null,
    };

    console.info("CYBRID_TITLE_USAGE", JSON.stringify({ mode: "review", ...usage, state: exam.state, searchType: exam.searchType, readiness: review.qc.foreclosureReadiness }));
    ...
    return NextResponse.json({
      code: message.startsWith("AUTO_DETECT_SEARCH_TYPE_FAILED:") ? "AUTO_DETECT_SEARCH_TYPE_FAILED" : status === 400 ? "UNSUPPORTED_SEARCH_TYPE" : "REVIEW_FAILED",
      error: message.replace(/^AUTO_DETECT_SEARCH_TYPE_FAILED:\s*/, ""),
